# Select

This field selects an option from a list of options.

import SelectField from "../../components/fields/SelectField";

<SelectField />

## Schema

import SchemaViewer from "../../components/SchemaViewer";
import schema from "../../components/fields/SelectField/schema.json";

<SchemaViewer value={schema} />

## Field Properties

| Name        | Description                                |
| ----------- | ------------------------------------------ |
| displayType | The displayType for this field (`select`)  |
| displayName | The display name used for this field       |
| options     | An list of available options for the field |
| value       | The value used by this field               |
| placeholder | The hint value used for this field         |
| isDisabled  | Marks the field as disabled                |
| isReadonly  | Marks the field as readonly                |

## Config

The configuration defines how to load dropdown options for the `select` field. Use this to load options from remote api and configure its behaviour.
| Name | Description |
| ----------- | ------------------------------------------ |
| url | The api endpoint to load dropdown options |
| pathParams | The path parameters of the url. Use this for dynamic path parameters |
| queryParams | List of query parameters. Use this for dynamic query parameters |
| requestHeaders | Custom headers specific to field which is used when calling api methods |
| requestType | Http request type to be used. Supported types are 'get', 'post' |
| requestBody | Payload body in json format to be used with 'post' request type |
| requstBodyParams | A list of dynamic parameter values used alongwith variables contained in requestBody |
| responseKey | The key from response which contains the results |
| valueKey | The key from the response for value of the field |
| labelKey | The key from the response for display name of the field |
| lazy | Load options lazily. For example: load the options on opening dropdown |

## Query params
Query params can be used to pass additional data as part of the api url. This data can either be hardcoded or configured to pass data dynamically from other fields.

**Format of query params:**
It accepts an array/list of query params.

```
[<QUERY_PARAM1>, <QUERY_PARAM2>,...]
```

**Format of query param:**
It accepts name and value in list like format.

```
[<NAME_OF_QUERY_PARAM>, <VALUE_OF_QUERY_PARAM>]
```

- NAME_OF_QUERY_PARAM is just a string
- VALUE_OF_QUERY_PARAM can be a hardcoded value or can reference value from another field like this:
  - type: 'fieldValue' indicates the value has to be taken from another field
  - ref: name of the reference field from where value can be copied

import schemaQueryParams from "../../components/fields/SelectField/schema-query-params.json";

### Schema example with query params

<SchemaViewer value={schemaQueryParams} />

## Request Body

- Request body is the payload of api request in key, value pair (json format ). 
- Request body may contain dynamica parameter variables.
- Parameter variables are in dollar notation like $0, $1, $2 etc. where $0 is the first parameter variable (0 indexed);

## Request Body Params

- It contains values of the parameter variables used in request body above.
- It is an array of values where the index of each value in the array corresponds to the parameter variable in request body.
e.g. -> The value of dynamic variable $2 (in request body) is found in index 2 of request body parameters.
- 

| DataType |
|----------|
| Array of values


## Common Properties

[Click here to go to common properties](/fields/common-properties)

## How to load options from remote api

import SelectWithCountries from "../../components/fields/SelectField/SelectWithCountries";
import schemaCountries from "../../components/fields/SelectField/schema-countries.json";

<SelectWithCountries />

## Schema

<SchemaViewer value={schemaCountries} />
