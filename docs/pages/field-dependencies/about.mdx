...
| exists       | Checks criteria whether to display field in the UI. This dependency is **trasitive** in nature. |
...
- In case current value is array, it will try to check if it contains any value matching the reference field value.
...
```typescript
type TCondition = [IOperand, TOperator, IOperand | TValue, TNextCondition?];
