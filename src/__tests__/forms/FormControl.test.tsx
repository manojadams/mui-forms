...
jest.mock("@manojadams/metaforms-core", () => {
    ...
    return {
        BaseFormControl: class extends React.Component {
            ...
            getWrapperClassName() { return "meta-wrapper"; }
            handleChange() {}
            handleValidation() {}
            setError() {}
            render() {
                const type = (this as any).field.type;
                if (type === "text") return (this as any).text();
                if (type === "select") return (this as any).select();
                if (type === "checkbox") return (this as any).checkbox();
                return null;
            }
        }
    };
...
jest.mock("../../components/InputControl", () => (props: any) => <div data-testid="input-control">{props.form.displayName}</div>);
jest.mock("../../components/SelectControl", () => (props: any) => <div data-testid="select-control">{props.form.displayName}</div>);
jest.mock("../../components/CheckboxControl", () => (props: any) => <div data-testid="checkbox-control">{props.form.displayName}</div>);
